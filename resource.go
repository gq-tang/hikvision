...
type (
	...
	ResourcesByParamsReq struct {
		...
		Expressions      []Expression `json:"expressions,omitempty"`      // 查询表达式
	}
)
...
type (
	...
	ResourceData struct {
		...
		PageNo   int            `json:"pageNo"`   // 当前页码
		PageSize int            `json:"pageSize"` // 分页大小
		List     []ResourceItem `json:"list"`     // 返回数据
	}
...
func (c *Client) ResourcesByParams(ctx context.Context, req *ResourcesByParamsReq) (*ResourcesByParamsResp, error) {
	response, err := c.doRequest(ctx, http.MethodPost, PathResourcesByParams, nil, nil, req)
	if err != nil {
		return nil, err
	}
